...
//      }
      ...
      float renderScaleX = (float) Screen.getMainScreen().getPlatformScaleX();
      float renderScaleY = (float) Screen.getMainScreen().getPlatformScaleY();
      float renderScale = Math.max(renderScaleX, renderScaleY);

      if (PApplet.platform == PConstants.MACOSX) {
        for (Screen s : Screen.getScreens()) {
          float scaleX = (float) s.getPlatformScaleX();
          float scaleY = (float) s.getPlatformScaleY();
          renderScale = Math.max(renderScale, Math.max(scaleX, scaleY));
        }
      }
      ...
      float uiScale = Math.max(renderScaleX, renderScaleY);
