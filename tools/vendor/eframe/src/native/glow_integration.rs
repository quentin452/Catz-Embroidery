    ...
    fn run_ui_and_paint(
        ...
        if is_visible {
...

            {
                ...
                // INSTRUMENTATION PATCH: reveal the window only AFTER the first
                // present has happened. Original eframe order (post_rendering
                // before swap_buffers) shows a white window while the first
                // present is slow (measured: ~665 ms on this hybrid GPU laptop).
